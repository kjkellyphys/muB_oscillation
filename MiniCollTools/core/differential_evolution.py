...
class DifferentialEvolutionSolver:
    ...
    def __init__(
        ...
        if modes is not None:
            self.modes = np.array(modes, dtype="float")
            if (
                len(self.modes.shape) != 2
                or np.size(self.modes, 1) != self.parameter_count
            ):
                raise ValueError(
                    "modes must be a sequence containing sets of linear coefficients for the parameters"
                )
        else:
            self.modes = None
        ...
        if center is not None:
            ...
            if (
                len(self.center.shape) != 1
                or np.size(self.center, 0) != self.parameter_count
            ):
                raise ValueError(
                    "center must be a sequence containing parameter central values"
                )
        else:
            self.center = None
    ...
    def solve(self):
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            if warning_flag or (self.converged() and nit >= self.miniter):
                break
    ...
    def _scale_size_parameters(self, trial):
        """Scale from a number between 0 and 1 to parameters."""
        return trial * self.__scale_arg2

    def _unscale_parameters(self, parameters):
        """Scale from parameters to a number between 0 and 1."""
        return (parameters - self.__scale_arg1) / self.__scale_arg2 + 0.5

    def _unscale_size_parameters(self, parameters):
        """Scale from parameters to a number between 0 and 1."""
        return parameters / self.__scale_arg2
    ...
    def _mode_trial_transform(self, trial):
        parameters = self._scale_parameters(trial)
        return np.sum(parameters[None, :] * self.modes, axis=1)

    def _mode_parameters_transform(self, parameters):
        return np.sum(parameters[None, :] * self.modes, axis=1)

    def _mode_untransform(self, mode_parameters):
        return self._unscale_size_parameters(
            np.sum(mode_parameters[:, None] * self.modes, axis=0)
        )

    def _mode_center_trial_transform(self, trial):
        parameters = self._scale_parameters(trial)
        return np.sum((parameters - self.center)[None, :] * self.modes, axis=1)

    def _mode_center_parameters_transform(self, parameters):
        return np.sum((parameters - self.center)[None, :] * self.modes, axis=1)

    def _mode_center_untransform(self, mode_parameters):
        return self._unscale_parameters(
            np.sum(mode_parameters[:, None] * self.modes, axis=0) + self.center
        )
    ...
    def _mutate(self, candidate):
        ...
        if self.modes is not None and self.center is None:
            do_modes = rng.uniform(size=1)
            if do_modes < 0.5:
                transform = self._mode_trial_transform
                untransform = self._mode_untransform
                n_indexes = len(self.modes)
            else:
                transform = lambda x: x
                untransform = lambda x: x
                n_indexes = self.parameter_count
        elif self.modes is not None and self.center is not None:
            do_modes = rng.uniform(size=1)
            if do_modes < 0.5:
                transform = self._mode_center_trial_transform
                untransform = self._mode_center_untransform
                n_indexes = len(self.modes)
            else:
                transform = lambda x: x
                untransform = lambda x: x
            ...
            untransform = lambda x: x
            n_indexes = self.parameter_count
        fill_point = rng.choice(n_indexes)

        trial = transform(trial)

        if self.strategy in ["currenttobest1exp", "currenttobest1bin"]:
            bprime = self.mutation_func(
                candidate, self._select_samples(candidate, 5), transform
            )
        else:
            bprime = self.mutation_func(self._select_samples(candidate, 5), transform)

        if self.strategy in self._binomial:
            crossovers = rng.uniform(size=n_indexes)
            ...
            crossovers = rng.uniform(size=n_indexes)
            crossovers = crossovers < self.cross_over_probability
            while i < n_indexes and crossovers[i]:
                trial[fill_point] = bprime[fill_point]
                fill_point = (fill_point + 1) % n_indexes
                i += 1
            trial = untransform(trial)
            return trial

    def _best1(self, samples, transform):
        """best1bin, best1exp"""
        r0, r1 = samples[:2]
        return transform(self.population[0]) + self.scale * (
            transform(self.population[r0]) - transform(self.population[r1])
        )

    def _rand1(self, samples, transform):
        """rand1bin, rand1exp"""
        r0, r1, r2 = samples[:3]
        return transform(self.population[r0]) + self.scale * (
            transform(self.population[r1]) - transform(self.population[r2])
        )

    def _randtobest1(self, samples, transform):
        """randtobest1bin, randtobest1exp"""
        r0, r1, r2 = samples[:3]
        bprime = transform(np.copy(self.population[r0]))
        bprime += self.scale * (transform(self.population[0]) - bprime)
        bprime += self.scale * (
            transform(self.population[r1]) - transform(self.population[r2])
        )
        return bprime

    def _currenttobest1(self, candidate, samples, transform):
        """currenttobest1bin, currenttobest1exp"""
        r0, r1 = samples[:2]
        bprime = transform(self.population[candidate]) + self.scale * (
            transform(self.population[0])
            - transform(self.population[candidate])
            + transform(self.population[r0])
            - transform(self.population[r1])
        )
        return bprime

    def _best2(self, samples, transform):
        """best2bin, best2exp"""
        r0, r1, r2, r3 = samples[:4]
        bprime = transform(self.population[0]) + self.scale * (
            transform(self.population[r0])
            + transform(self.population[r1])
            - transform(self.population[r2])
            - transform(self.population[r3])
        )

        return bprime

    def _rand2(self, samples, transform):
        """rand2bin, rand2exp"""
        r0, r1, r2, r3, r4 = samples
        bprime = transform(self.population[r0]) + self.scale * (
            transform(self.population[r1])
            + transform(self.population[r2])
            - transform(self.population[r3])
            - transform(self.population[r4])
        )
