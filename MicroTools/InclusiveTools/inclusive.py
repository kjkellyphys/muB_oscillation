...
BkgVec = np.load(f"{micro.muB_inclusive_data_path}/TotalBackground.npy")

# Expected Low-Energy-Excess rate (full strength) in the inclusive channel, 25 bins between 0 and 2.5 GeV
LEEVec = np.load(f"{micro.muB_inclusive_data_path}/LEE_x1_Expectation.npy")

# Observed inclusive-channel data, 25 bins between 0 and 2.5 GeV
DatVec = np.load(f"{micro.muB_inclusive_data_path}/ObservedData.npy")

# Correlation matrix amongst [25, 25] signal bins
SigCorr = np.load(f"{micro.muB_inclusive_data_path}/SigCorr_PD.npy")
