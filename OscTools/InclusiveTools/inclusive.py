...
import OscTools.micro_tools as micro

# Expected total background rate in the inclusive channel, 25 bins between 0 and 2.5 GeV
BkgVec = np.load(
    files(f"{micro.muB_inclusive_data_path}").joinpath("TotalBackground.npy").open()
)

# Expected Low-Energy-Excess rate (full strength) in the inclusive channel, 25 bins between 0 and 2.5 GeV
LEEVec = np.load(
    files(f"{micro.muB_inclusive_data_path}").joinpath("LEE_x1_Expectation.npy").open()
)

# Observed inclusive-channel data, 25 bins between 0 and 2.5 GeV
DatVec = np.load(
    files(f"{micro.muB_inclusive_data_path}").joinpath("ObservedData.npy").open()
)

# Correlation matrix amongst [25, 25] signal bins
SigCorr = np.load(
    files(f"{micro.muB_inclusive_data_path}").joinpath("SigCorr_PD.npy").open()
)
...
PoC_Uncertainty = np.load(
    f"{micro.muB_inclusive_data_path}/PostConstraint_Uncertainty.npy"
)
...
def Chi2_Inclusive(temp):
    ...
    for jj in range(nbins):
        for kk in range(nbins):
            sc0[jj][kk] += (
                np.sqrt(
                    ((PoC_Uncertainty[jj] - 1.0) * MV[jj])
                    * ((PoC_Uncertainty[kk] - 1.0) * MV[kk])
                )
                * SigCorr[jj][kk]
            )
...
def Chi2_Inclusive_0pXpi(temp):
    """
    Returns MicroBooNE Chi-Squared for 0pXpi Inclusive Channel, given model template temp
    """
    if np.size(temp) == 24:
        temp = np.append(temp, [0])
    v0 = DatVec - (BkgVec + temp)
...
def Poisson_Inclusive(AiVec, *info):
    ...
    TS0 = np.sum(
        [
            PLL(DV0[kk], np.max([0.0, (1.0 + AiVec[kk]) * MV[kk]]))
    ...
    for jj in range(nbinsP):
        for kk in range(nbinsP):
            sc0[jj][kk] += (
                np.sqrt(
                    ((PoC_Uncertainty[jj] - 1.0) * (1.0 + AiVec[jj]) * MV[jj])
                    * ((PoC_Uncertainty[kk] - 1.0) * (1.0 + AiVec[kk]) * MV[kk])
                )
                * SigCorr[jj][kk]
            )
...
def Chi2_Inclusive_Poisson(temp, nbinsP):
    ...
    res0 = minimize(
        Poisson_Inclusive,
        zeroVec0,
        args=[DatVec, BkgVec, temp, nbinsP],
        options={"disp": False, "direc": AiVec1},
        tol=0.0001,
        method="Powell",
    )
    return res0.fun
