...
Ereco_nue, Etrue_nue, Length_nue, Weight_nue = apps.get_MC_from_data_release_nue(
    mode="fhc", year="2022"
)
Ereco_numu, Etrue_numu, Length_numu, Weight_numu = apps.get_MC_from_data_release_numu(
    mode="fhc", year="2022"
)

Ereco_nuebar, Etrue_nuebar, Length_nuebar, Weight_nuebar = (
    apps.get_MC_from_data_release_nue(mode="rhc", year="2022")
)
Ereco_numubar, Etrue_numubar, Length_numubar, Weight_numubar = (
    apps.get_MC_from_data_release_numu(mode="rhc", year="2022")
)
...
def MiniBooNEChi2_deGouvea(
    ...
    MBSig_for_MBfit_bar = np.dot(
        ...
        apps.migration_matrix_official_bins_nuebar_11bins,
    )
...
def get_nue_rates(
    ...
    if disappearance:
        ...
        if include_antineutrinos:
            Weight_nuebar_flux = apps.reweight_MC_to_nue_flux(
        ...
        if use_numu_MC:
            ...
            dic["MC_numu_bkg_total_w_dis"] = np.dot(
                ...
                apps.migration_matrix_official_bins_numu,
            )
            if include_antineutrinos:
                ...
                dic["MC_numubar_bkg_total_w_dis"] = np.dot(
                    ...
                    apps.migration_matrix_official_bins_numubar,
                )
