...
def RelativeNorm():
    ...
    for i in range(number_of_bins):
        WBkg[i] = (
            (BinBound[i + 1] - BinBound[i])
            * (Other[i] + Dirt[i] + Delta[i] + Pi0[i] + NuEK0[i] + NuEKp[i] + NuEM[i])
            / mbfit.mb_nue_analysis_predicted_background[i]
        )
...
def CalculateNewBackgrounds(n_background_vector):
    ...
    eta0 = (
        SumDa
        - n_background_vector[0] * SumDl
        - n_background_vector[1] * SumPi
        - n_background_vector[2] * SumEn
        - SumExcs
    ) / SumOD
    if eta0 < 0:
    ...
    for i in range(number_of_bins):
        background_neutrino[i] = ((BinBound[i + 1] - BinBound[i]) / WBkg[i]) * (
            (Other[i] + Dirt[i]) * eta0
            + Delta[i] * n_background_vector[0]
            + Pi0[i] * n_background_vector[1]
            + (NuEK0[i] + NuEKp[i] + NuEM[i]) * n_background_vector[2]
        )
...
def GetLikelihood(parameters, background_model=False):
    ...
    # mub_signal_vector = np.append(0.0, converter.unfold(template))
    # chi2_mub = inclusive.Chi2_Inclusive(mub_signal_vector)

    return -chi2_mb / 2.0  # converting from chi^2 to gaussian likelihood
...
def run_mcmc(
    ...
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, lnprob, threads=threads, args=[nbins, background_model]
    )
    ...
    if background_model:
        ...
        if nbins < 11:
            complete_templates = np.concatenate(
                (
                    sampler.flatchain,
                    np.zeros((np.shape(sampler.flatchain)[0], 11 - nbins)),
                ),
                axis=1,
            ).T
        else:
    ...
    np.savetxt(
        f"{PATH_MCMC_CHAINS}/miniboone_signal_chain_{nbins}_bins_{appendix}.dat",
        complete_templates,
    )
...
def new_template(x, n=2):
    ...
    bin_size_correction = mbfit.bin_width[recipients] / np.sum(
        mbfit.bin_width[recipients]
    )
    # bin_size_correction = BF_template[recipients]/np.sum(BF_template[recipients])
    BF_template[recipients] += (
        np.sum(BF_template[:n] - downscaled_bins) * bin_size_correction
    )
...
def filter_samples(samples, p_threshold=0.1):
    ...
    for template in samples:
        if (
            get_pvalue(template) > p_threshold
        ):  # and template[0]>mbfit.mb_nominal_excess[0]:
...
def get_max_avgenu_templates(samples, n=4):
    avgenus = np.sum(
        (samples.T * mbfit.bin_centers).T / np.sum(samples, axis=0), axis=0
    )
