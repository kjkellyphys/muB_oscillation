...
import OscTools as MT
from OscTools import unfolder
from OscTools.InclusiveTools import inclusive
from OscTools.TemplateTools import mcmc_signal
from OscTools.TemplateTools import miniboone_fit as mbfit
...
class Templates:
    def __init__(self, case="signal", nbins=5, name=""):
        ...
        self.name = name
        self.nbins = nbins
        self.template_path = f"{MT.PATH_MCMC_CHAINS}/miniboone_signal_chain_{self.nbins}_bins_example.dat"

        # properties of nominal template
        self.nominal_nevents = 360
        self.nominal_template = (
            mbfit.mb_nue_analysis_data - mbfit.mb_nue_analysis_predicted_background
        )
        ...
        self.mb_sterile_best_fit = (
            SterileBF - mbfit.mb_nue_analysis_predicted_background
        )
    ...
    def load_templates(self, force_path=""):
        ...
        self.avg_energy = (
            np.matmul(self.mcmc_chain, mbfit.bin_centers)
            / self.signal_strength
            / self.nominal_nevents
        )
    ...
    def unfold(self, **args):
        ...
        self.converter = unfolder.MBtoLAr(**args)

        # unfold all tempaltes
        self.micro_templates = np.empty(0)
        for template in self.mcmc_chain:
            template[template < 0] = 0.0
            self.micro_templates = np.append(
                self.micro_templates, np.append(0.0, self.converter.unfold(template))
            )

        self.micro_templates = np.reshape(
            self.micro_templates, (np.shape(self.mcmc_chain)[0], 25)
        )

        self.micro_nominal = np.append(
            0.0, self.converter.unfold(self.nominal_template)
        )

        # compute the ratio of events of new tempalte with nominal one at MicroBooNE
        self.mub_signal_strength = np.sum(self.micro_templates, axis=1) / np.sum(
            self.micro_nominal
        )

        # MicroBoonE average neutrino energy
        self.mub_avg_energy = np.matmul(
            self.micro_templates, inclusive.BinCenter * 1e3
        ) / np.sum(self.micro_nominal)
    ...
    def compute_chi2s(self):
        ...
        for mb_template, mub_template in zip(self.mcmc_chain, self.micro_templates):
            ...
            self.mb_chi2s = np.append(
                self.mb_chi2s, mbfit.CalculateChi2SignalExpectations(mb_template)
            )
        ...
        for template in self.micro_templates:
            ...
            self.mub_chi2s = np.append(
                self.mub_chi2s, inclusive.Chi2_Inclusive(template)
            )
    ...
    def compute_MB_pval(self, ndf=4.7):
        ...
        for template in self.mcmc_chain:
            self.mb_pval = np.append(
                self.mb_pval, mcmc_signal.get_pvalue(template, ndf=ndf)
            )
