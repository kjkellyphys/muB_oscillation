...
def GetRescaledCovarianceMatrix(
    fractional_covariance, predicted_signal, predicted_background, predicted_numu
):
    signal_prediction_matrix = np.diag(
        np.concatenate(
            [predicted_signal, predicted_background * 0.0, predicted_numu * 0.0]
        )
    )
    total_prediction_matrix = np.diag(
        np.concatenate([predicted_signal, predicted_background, predicted_numu])
    )
    # miniboone covariance matrix is fractional, this rescale to physical rates
    rescaled_cov_matrix = np.dot(
        total_prediction_matrix, np.dot(fractional_covariance, total_prediction_matrix)
    )
    return (
        rescaled_cov_matrix + signal_prediction_matrix
    )  # this adds the statistical error on data
...
def CalculateChi2(
    ...
    rescaled_covariance = GetRescaledCovarianceMatrix(
        fractional_covariance, by_hand_mod, predicted_background, predicted_numu
    )
    ...
    assert np.abs(np.sum(covariance) - np.sum(rescaled_covariance)) < 1.0e-3

    # compute residuals
    residuals = np.concatenate(
        [
            observed_nue - (by_hand_mod + predicted_background),
...
def CalculateChi2BackgroundExpectations(predicted_neutrino_background):
    ...
    return CalculateChi2(
        fractional_covariance_matrix,
        predicted_neutrino_signal,
        mb_nue_analysis_predicted_background,
        predicted_neutrino_numu,
        observed_neutrino_nue,
        observed_neutrino_numu,
    )
