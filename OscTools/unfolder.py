...
##################################################################
# unfolding
path_unfolding_data = f"OscTools.include.muB_data.unfolding_data"
...
class MBtoLAr:
    def __init__(
        ...
        self._migration = np.loadtxt(
            files(path_unfolding_data)
            .joinpath("MiniBooNE_migration_matrix_paper_bins.dat")
            .open()
        )  # MiniBooNE migration matrix
        self.MB_eff = np.sum(self._migration, axis=0)
        self._MB_MC = np.loadtxt(
            files(path_unfolding_data).joinpath("MC_truth.dat").open()
        ...
        if analysis == "CCQE":
            ...
            self._smearing_matrix_microB = np.loadtxt(
                files(path_unfolding_data).joinpath("Migration_CCQE.dat").open()
            )
            ...
            self._smearing_matrix_microB = np.loadtxt(
                files(path_unfolding_data).joinpath("Migration_1eX.dat").open()
            )
            ...
            self._smearing_matrix_microB = np.loadtxt(
                files(path_unfolding_data).joinpath("Migration_1eX_PC.dat").open()
            )
        elif analysis == "SBND" or analysis == "ICARUS":

            # SBND/MB
            self._relative_exposure = 1
            self._relative_targets = apps.rescale_mini_to_SBN(analysis, which_template)

            if not self._unfold:
                self._relative_targets *= (osc.L_micro / osc.L_mini) ** 2
...
#         # Set up data needed for unfolding ##
#         self._MB_MC = np.loadtxt(
#          files(f"{micro)path_unfolding_dat").joinpath("MC_truth.dat").open()
#         )  # MiniBooNE nu_e background. To be used as a prior for unfolding
...
#         self._migration = np.loadtxt(
#          files(f"{micro)path_unfolding_dat").joinpath("MiniBooNE_migration_matrix_paper_bins.dat").open()
#         )  # MiniBooNE migration matrix
#         self.nu_MB_eff = np.sum(self._migration, axis=0)

#         # antineutrino migration matrix
#         self._nubar_migration = np.load(
#             f"{path_unfolding_antinu_data}/CH2_f0.17.npy"
#         )  # MiniBooNE migration matrix
#         eff_data = np.load(f"{path_unfolding_antinu_data}MB_efficiency.npy")
...
#             np.load(f"{path_unfolding_antinu_data}/CH2_Etrue_range.npy") * 1e3
...
#              files(path_unfolding_data).joinpath("Migration_1eX.dat").open()
#             )
#            self._nubar_smearing_matrix_microB)= np.loadtxt(files(path_unfolding_data).joinpath("our_antinu_Migration_1eX.dat")).open()
#             self._nubar_smearing_matrix_microB = np.load(
#                 f"{path_unfolding_antinu_data}/Ar40_f0.12.npy"
#             )[1:-1, :]
