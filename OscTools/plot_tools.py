...
from . import apps
from .mini_tools import bin_width
from .micro_tools import muB_inclusive_datarelease_path
...
def make_micro_rate_plot(
    ...
    other_bkg = np.load(
        ...
        .joinpath(f"nueCC_{SAMPLE}_Bkg.npy")
        .open("rb")
    )
    intrinsic_bkg = np.load(
        files(muB_inclusive_datarelease_path)
        .joinpath(f"nueCC_{SAMPLE}_Sig.npy")
        .open("rb")
    )
    data = np.load(
        files(muB_inclusive_datarelease_path)
        .joinpath(f"nueCC_{SAMPLE}_Obs.npy")
        .open("rb")
    )
...
def make_SBN_rate_plot(
    ...
    SBN = SBN * (
        sterile.PmeoscAvg(bins[:-1], bins[1:], osc.L_SBND)
        / sterile.PmeoscAvg(bins[:-1], bins[1:], osc.L_mini)
    )
    ICARUS = ICARUS * (
        sterile.PmeoscAvg(bins[:-1], bins[1:], osc.L_ICARUS)
        / sterile.PmeoscAvg(bins[:-1], bins[1:], osc.L_mini)
    )
    ...
    other_bkg = other_bkg * apps.rescale_micro_to_SBN(detector)
    intrinsic_bkg = intrinsic_bkg * apps.rescale_micro_to_SBN(detector)
