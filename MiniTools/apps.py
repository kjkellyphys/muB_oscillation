...
def get_MC_from_data_release_nue(mode="fhc", year="2020"):
    ...
    if year == "2022":
        Ereco, Etrue, Length, Weight = pickle_read(
            f"MiniTools/include/MB_data_release_{year}/{mode}mode/miniboone_numu{bar}nue{bar}fullosc_ntuple.pkl"
        ).T
    else:
        MiniBooNE_Signal = np.loadtxt(
            f"MiniTools/include/MB_data_release_{year}/{mode}mode/miniboone_numu{bar}nue{bar}fullosc_ntuple.txt"
        )
...
def get_MC_from_data_release_numu(mode="fhc", year="2022"):
    ...
    if year == "2022":
        Ereco, Etrue, Length, Weight = pickle_read(
            f"MiniTools/include/MB_data_release_{year}/{mode}mode/miniboone_numu{bar}fullosc_ntuple.pkl"
        ).T
    elif year == "2009":
